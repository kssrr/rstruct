...
class HashSet {
 ...
 public:
  ...
  HashSet(const RType& input) {
    ...
    data_.reserve(n);
    for (R_xlen_t i = 0; i < n; i++) data_.insert(ElemApi::fromR(input[i]));
  }

  bool contains(typename ElemApi::ArgType q) const { return data_.find(ElemApi::toKey(q)) != data_.end(); }

  void insert(typename ElemApi::ArgType v) { data_.insert(ElemApi::toKey(v)); }

  void remove(const RType& V) {
    for (R_xlen_t i = 0; i < V.size(); i++) data_.erase(ElemApi::fromR(V[i]));
  }

  Rcpp::LogicalVector lookup(const RType& Q) const {
    R_xlen_t n = Q.size();
    Rcpp::LogicalVector res(n);
    for (R_xlen_t i = 0; i < n; i++) res[i] = data_.find(ElemApi::fromR(Q[i])) != data_.end();
    return res;
  }

  void update(const RType& V) {
    for (R_xlen_t i = 0; i < V.size(); i++) data_.insert(ElemApi::fromR(V[i]));
  }

  void print() const { Rcpp::Rcout << "C++ hash set with " << data_.size() << " elements.\n"; }
};
